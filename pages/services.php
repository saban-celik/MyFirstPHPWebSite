<!DOCTYPE html>
<html lang="tr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Hizmetler</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="css/style.css">
    <link rel="stylesheet" href="css/services.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
</head>
<body>
    <div class="main-services-div">
        <div class="services-one-content-all">
            <div class="services-one-content container">
                <h2>Hizmetler</h2>
                <p>Profesyonel fotoğrafçılık ve video alanında müşterilerimize özel çözümler sunarak, en iyi ve profesyonel şekilde geniş bir hizmet yelpazesi sunmaktayız.</p>
            </div>
        </div>
        <div class="services-two-content-all">
            <div class="services-two-content">
                <div class="container">
                    <div class="row" id="services-cards">
                        <!-- Kartlar buraya eklenecek -->
                    </div>
                </div>
            </div>
        </div>
    </div>

    <script src="js/script.js"></script>
</body>
</html>
